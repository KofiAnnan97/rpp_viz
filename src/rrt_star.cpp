...
void RRTStar::solve(pair<int,int> sp, pair<int,int> ep){
    ...
    for(int i = 0; i < max_iter; i++){
        ...
        if(tree.is_node_valid(new_node) == true){
            auto neighbors = find_neighbors(node_list, new_node); //tree.get_edges_without_weights(new_node);
        ...
        if(RRTStar::euclidean_distance(new_node,ep) <= 1.5){
            goal_reached = true;
            if(temp_parent[new_node] != ep) {
                temp_parent[ep] = new_node;
                cost_map[ep] = cost_map[new_node] + euclidean_distance(new_node,ep);
            }
            break;
        }    
    }
    /*for(auto pair: temp_parent){
        auto key = pair.first;
...
pair<int,int> RRTStar::get_random_node(){
    ...
    if(r > 0.2) {
        //random_node = {rand()%height, rand()%width};
        //cout << all_valid_nodes.size() << endl;
        int r_idx = rand()%all_valid_nodes.size();
        //std::default_random_engine generator;
        //std::uniform_real_distribution<float> distribution(0.0,tree.get_size());
...
pair<int,int> RRTStar::steer(pair<int,int> from_node, pair<int,int> to_node){
    ...
    temp_parent[new_node] = from_node;
    return new_node;
}
...
pair<int,int> RRTStar::choose_parent(vector<pair<int,int>> neighbors, pair<int,int> nearest_node, pair<int,int> new_node){
    ...
    temp_parent[new_node] = best_node;
    return new_node;
}
