...
void PathWorker::run_bfs(const Graph &g){
    try {
        auto bfs = BFS(g);
        ...
        {
            QMutexLocker locker(&timeout_mutex);
        ...
        qWarning() << "BFS algorithm exception:" << e.what();
    }
}

// A* algorithm module
void PathWorker::run_a_star(const Graph &g){
    try {
        auto as = AStar(g);
        auto start_time = high_resolution_clock::now();
        ...
        qWarning() << "A* algorithm exception:" << e.what();
    }
}

// RRT* algorithm module
void PathWorker::run_rrt_star(const Graph &g, int sample_count, int step_size){
    try {
        ...
        qWarning() << "RRT* algorithm exception:" << e.what();
    }
}

// PRM algorithm module
void PathWorker::run_prm(const Graph &g, int sample_count, int neighbor_count){
    try {
        ...
        qWarning() << "PRM algorithm exception:" << e.what();
    }
}

// Compute path(s)
void PathWorker::compute_path(const QString &algo_name, const Graph &g, const SampleCountByAlgo &samples, int neighbor_count, int step_size){
    try {
        {
            QMutexLocker locker(&result_mutex);
            results.clear();
        ...
        qWarning() << "compute_path exception:" << e.what();
        {
            QMutexLocker locker(&result_mutex);
            emit compute_error(results, QString("Fatal error: %1").arg(e.what()));
        }
    }
}
